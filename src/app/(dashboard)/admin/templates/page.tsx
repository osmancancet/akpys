...
export default function AdminTemplatesPage() {
    ...
    const selectedCourseLecturer = courses.find(c => c.id === selectedCourse)?.lecturer?.fullName || "";
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-slate-400">Tüm dersler için YÖKAK uyumlu Excel şablonlarını indirin</p>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
                ...
                <select
                    ...
                    className="w-full md:w-2/3 bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-red-500 focus:border-transparent"
                >
                    <option value="">Genel Şablon (Ders seçilmedi)</option>
                    {courses.map((course) => (
                        <option key={course.id} value={course.id}>
                            {course.code} - {course.name} ({course.lecturer?.fullName || "Atanmamış"})
                        </option>
                    ))}
                </select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <TemplateCard
                    ...
                    lecturer={selectedCourseLecturer}
                    examType="Vize"
                />
...
function TemplateCard({
    title, description, features, color, icon, downloadUrl, courseName, courseCode, lecturer, examType
}: {
    title: string;
    description: string;
    features: string[];
        ...
        <div className={`${c.bg} border ${c.border} rounded-xl p-6`}>
            ...
            {courseName && (
                <div className="mb-4 p-3 bg-slate-700/50 rounded-lg">
                    ...
                    {lecturer && <p className="text-xs text-slate-400 mt-1">Öğr. Gör.: {lecturer}</p>}
                </div>
            )}
